...
async function main() {
  ...
  const dir = path.resolve(process.argv[3] || './metadata');
  try {
    const files = await getFilesFromPath(dir);
    const client = new Web3Storage({ token });
    console.log(`Uploading metadata directory ${dir}...`);
    const cid = await client.put(files, { wrapWithDirectory: true });
    console.log(`CID:${cid}`);
    console.log(`ipfs://${cid}/contract-metadata.json`);
    console.log(`ipfs://${cid}/logo.svg`);
    process.exit(0);
  } catch (err) {
